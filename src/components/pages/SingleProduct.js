...
const SingleProduct = ({ setProgress }) => {
  ...
  const { id } = useParams();
  const { products, CalcDiscount } = useProductContext();  // Destructure
  ...
  return (
    <div className='SingleProduct py-5'>
      {Data.map((item, index) => {
        const { id, name, description, discount, price, image, color, productMeta, quantity } = item; // Destructure From Object.
              ...
              <div className="col-6">
                ...
                <div className="SingleProduct_DetailsFooter">
                  ...
                  <div className="btnCol row m-0 p-0 ">
                    ...
                    <Link to="/checkout" className='btn BuyNowBtn outline-0 shadow-none rounded-0 col-6' onClick={() => { addToCart(id, color, quantity, item) }}>Buy It Now</Link>
