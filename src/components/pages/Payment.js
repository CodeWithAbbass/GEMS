...
const Payment1 = () => {
  ...
  const [paymentInfo, setPaymentInfo] = useState(1);
  const [orderInst, setOrderInst] = useState({ instructions: '' });
  const userMeta = user.filter(item => item.id === 123456)
  const { fName, lName, address, city, state, country, phone } = userMeta[0];
 
  const onChange = (e) => {
    const { name, value } = e.target;
    setOrderInst({ ...orderInst, [name]: value });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    setOrderInst({ instructions: '' });
    Navigate("/");
  };


  const PaymentInfoMethod = (val) => {
    setPaymentInfo(val);
  };
  ...
  return (
    <div className='Payment1 Checkout1 container'>
      <CheckoutProgressbar />
      <div className="Checkout_MainContainer row justify-content-between">
        <div className="formContainer col-8 pe-lg-5">
          <form className="CheckoutForm mb-3 row px-4" id="CheckoutFormId" action='#' method='POST' onSubmit={handleSubmit} >
            <h3 className='pb-2 mb-1'>Payment Method</h3>
            <div className="form-check">
              <input className="form-check-input rounded-circle" type="checkbox" name="flexRadioDefault" id="flexRadioDefault1" checked={paymentInfo === 1 ? true : false} onChange={() => PaymentInfoMethod(1)} />
              <label className="form-check-label" htmlFor="flexRadioDefault1"> Cash On Delivery</label>
            </div>
            <div className="form-check">
              <input className="form-check-input p-1 rounded-circle" type="checkbox" name="exampleRadios" id="flexRadioDefault2" checked={paymentInfo === 2 ? true : false} onChange={() => PaymentInfoMethod(2)} />
              <label className="form-check-label" htmlFor="flexRadioDefault2">Online Visa/MasterCard</label>
            </div>
            <div className="form-check">
              <input className="form-check-input p-1 rounded-circle" type="checkbox" name="exampleRadios" id="flexRadioDefault3" checked={paymentInfo === 3 ? true : false} onChange={() => PaymentInfoMethod(3)} />
              <label className="form-check-label" htmlFor="flexRadioDefault3">UnionPay Debit/Credit Cards</label>
            </div>
            <div className="form-check">
              <input className="form-check-input p-1 rounded-circle" type="checkbox" name="exampleRadios" id="flexRadioDefault4" checked={paymentInfo === 4 ? true : false} onChange={() => PaymentInfoMethod(4)} />
            ...
            <div className="my-3">
              ...
              <textarea className="form-control rounded-0 shadow-none mb-3" id="exampleFormControlTextarea1" name='instructions' value={orderInst.instructions} rows="3" maxLength={300} placeholder='Do you have any special instructions regarding the order?' onChange={onChange}></textarea>
                        ...
                        {cart.map((curItem, index) => {
                          ...
                          return (
                            <div className="Checkout_ProductInfo row mb-4" key={index}>
                              <div className="row">
                                <div className="col-3"><img src={image} alt="Product" className='w-100' /></div>
                                <div className="col-6">
          ...
          <div className="MobileOrderDetailsSidebar">
            <div className="DesktopOrderSummery MobileOrderSummerySidebar px-2 py-2">
              ...
              <div className="ShipTo bg-white mt-4">
                ...
                <div className="ShipTo_Body">
                  <p className='py-1'>{userMeta.fName} {userMeta.lName}</p>
                  <p className='py-1'>{userMeta.address}</p>
                  <p className='py-1'>{userMeta.city}, {userMeta.state}</p>
                  <p className='py-1'>{userMeta.country}</p>
                  <p className='py-1'>{userMeta.phone}</p>
                </div>
                <div className="ShipTo_Footer mt-4">
                  <div className="ShipTo_Header mb-2 d-flex justify-content-between align-items-center">
                    <p className='ShipTo_heading mb-2'>Shipping Method</p>
